/****************************************************
    文件：PathDefine.cs
	作者：Happy-11
    日期：2021年1月16日23:58:31
	功能：路径配置
*****************************************************/

using UnityEngine;

public class PathDefine : MonoBehaviour 
{
    public const string spriteStartMusic = "ResUI/LoginUI/音乐正常播放";
    public const string spriteStopMusic= "ResUI/LoginUI/音乐暂停";

    #region PrefabsPath
    //btn
    public const string btnTrustDetailPrefab= "ResPrefab/Item/btn/btnTrustDetail";
    public const string btnElementPrefab = "ResPrefab/Item/btn/btnElement";
    public const string btnCustomerPrefab = "ResPrefab/Item/btn/btnCustomer";
    public const string btnContractPrefab = "ResPrefab/Item/btn/btnContract";
    public const string btnTrustPrefab = "ResPrefab/Item/btn/btnTrust";
    public const string btnPresentPrefab = "ResPrefab/Item/btnPresent";
    public const string btnNullPrefab = "ResPrefab/Item/btn/btnNull";



    #endregion



}